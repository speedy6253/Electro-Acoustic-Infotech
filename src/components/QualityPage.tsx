...
export const QualityPage: React.FC<QualityPageProps> = ({ onOpenConsultation }) => {
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-16">
      {/* Header Banner - Light Corporate Theme */}
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {METRICS.map((metric) => (
            <div
              key={metric.id}
              className="bg-white rounded-2xl p-6 border border-slate-200 shadow-xs space-y-2 hover:border-emerald-300 transition-all"
            >
              <div className="text-emerald-700 font-extrabold text-2xl tracking-tight">
                {metric.value}
              </div>
              <div className="font-bold text-sm text-slate-900">{metric.label}</div>
              <p className="text-xs text-slate-600 leading-relaxed">{metric.subtext}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16 space-y-10">
        <div className="text-center max-w-2xl mx-auto space-y-2">
          <span className="text-xs font-bold text-emerald-800 uppercase tracking-wider bg-emerald-50 px-3 py-1 rounded-full border border-emerald-200 inline-block mb-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="bg-white rounded-2xl p-6 border border-slate-200 space-y-4 shadow-xs hover:border-emerald-300 transition-all">
            <div className="w-11 h-11 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-600 flex items-center justify-center font-bold">
              <FileCheck className="w-5.5 h-5.5" />
            </div>
            <h3 className="font-bold text-base text-slate-900">1. Acoustic Design Verification</h3>
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-200 space-y-4 shadow-xs hover:border-emerald-300 transition-all">
            ...
            <h3 className="font-bold text-base text-slate-900">2. OEM Component Assurance</h3>
            <ul className="space-y-2 text-xs text-slate-600">
              <li className="flex items-start gap-2">
                <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-200 space-y-4 shadow-xs hover:border-emerald-300 transition-all">
            ...
            <h3 className="font-bold text-base text-slate-900">3. On-Site Acceptance Testing</h3>
            <ul className="space-y-2 text-xs text-slate-600">
              <li className="flex items-start gap-2">
                <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16">
        <div className="bg-gradient-to-br from-white via-emerald-50/50 to-slate-50 rounded-3xl p-8 border border-emerald-200 shadow-xs flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="space-y-2 max-w-xl">
            <div className="flex items-center gap-2 text-emerald-800 font-bold text-xs uppercase tracking-wider">
              <ShieldCheck className="w-4 h-4 text-emerald-600" />
              <span>Government Registration & Quality Credentials</span>
            </div>
            <h3 className="text-xl sm:text-2xl font-bold text-slate-900">{COMPANY_INFO.govRegistration}</h3>
            <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
          ...
          <button
            ...
            className="px-6 py-3.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow-2xs transition-all shrink-0"
          >
